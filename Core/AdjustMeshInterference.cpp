...
bool CAdjustMeshInterference::AdjustMesh( CTextile &Textile, vector<CMesh> &YarnMeshes, double Tolerance ) {
	...
	m_IntersectionMeshes.resize(Textile.GetNumYarns());
	m_Tolerance = Tolerance;
	// Mesh.CalculateVolume() before & after gives change in total yarn volume.
	//May need to use if output to format where using Vf = 1 and just need to know how much mass has been removed
	for (int i = 0; i < Textile.GetNumYarns(); i++) {
		TGLOG( "Volume of yarn mesh " << i << " = " << YarnMeshes[i].CalculateVolume() << "\n" );
	}
	m_TempYarnMeshes.clear();
	for (vector<CMesh>::iterator itMesh = YarnMeshes.begin(); itMesh != YarnMeshes.end(); ++itMesh ) {
		m_TempYarnMeshes.push_back(*itMesh);
	}
	// Returns false if any outside tolerence so adjustement needed
	// Returns false any of intersections larger than element intersecting with
	if ( !CheckInitialIntersections(Textile, YarnMeshes) ) {
		return AdjustInitialIntersections(YarnMeshes)
			&& AdjustIntersections(YarnMeshes);
	}
	else {
		return true;
	}
...
bool CAdjustMeshInterference::CheckInitialIntersections( CTextile &Textile, vector<CMesh> &YarnMeshes ) {
	...
	// Simulation Abaqus only uses repeats from yarn 0 to get surface.
	// Makes assumption that repeats are same for all yarns (valid?...)
	const vector<XYZ> &Repeats = Textile.GetYarn(0)->GetRepeats();
	for (size_t i = 0; i < InterferenceDepths.size(); i++) {
		double dMinDist = -1.0;
		double dDist = fabs(InterferenceDepths[i]);
		// Find max interference depth
		if (dDist > dMaxDist) {
			dMaxDist = dDist;
		}
		int iIndex = YarnMeshes[YarnIndices[i]].GetClosestNode( InterferencePoints.GetNode( i ) );
		if ( iIndex != -1 ) {
			...
			for (vector<XYZ>::const_iterator itRepeat =Repeats.begin(); itRepeat!=Repeats.end(); ++itRepeat) {
	...
	// Intersections all within tolerance
	return dMaxDist < m_Tolerance;
}
...
bool CAdjustMeshInterference::AdjustInitialIntersections( vector<CMesh> &YarnMeshes ) {
	...
	for (vector<CMeshIntersectionData>::iterator itIntersections = m_Intersections.begin();
		itIntersections != m_Intersections.end(); ++itIntersections ) {
		...
		bool bFoundNode = itIntersections->FindInterpolationNode( YarnMeshes[iYarn]);

		if (bFoundNode) {
			// Adjust the point	
			if (!itIntersections->MoveNode(YarnMeshes[iYarn])) {
				return false;   // Adjustment size > element size
			}
		}
	}
	// Can't do this in previous loop because need to wait until all nodes adjusted
	for (vector<CMeshIntersectionData>::iterator itIntersections = m_Intersections.begin();
		itIntersections != m_Intersections.end(); ++itIntersections ) {
		// Add the elements to a new mesh for the appropriate yarn to be used for further intersection testing
		AddIntersectElementsToMesh( itIntersections->GetElements(), itIntersections->GetYarn(), YarnMeshes[itIntersections->GetYarn()] );
	}
	for (vector<CMesh>::iterator itMesh = m_IntersectionMeshes.begin(); itMesh != m_IntersectionMeshes.end(); ++itMesh) {
		itMesh->MergeNodes();
...
void CAdjustMeshInterference::AddIntersectElementsToMesh( const vector<ELEMENT_INDICES> &Elements, int iYarn, CMesh &YarnMesh ) {
	for (vector<ELEMENT_INDICES>::const_iterator itElements = Elements.begin(); itElements != Elements.end(); ++itElements ) {
...
bool CAdjustMeshInterference::AdjustIntersections( vector<CMesh> &YarnMeshes ) {
	...
	for (vector<CMesh>::iterator itIntersectionMeshes = m_IntersectionMeshes.begin();
		itIntersectionMeshes != m_IntersectionMeshes.end(); itIntersectionMeshes++ ) {
		MeshAABB.push_back(itIntersectionMeshes->GetAABB());
	}
...
void CAdjustMeshInterference::SetNodeDisplacements( int iNumYarns, vector<CMesh> &YarnMeshes ) {
	...
	for (vector<CMeshIntersectionData>::iterator itMeshData = m_Intersections.begin(); itMeshData != m_Intersections.end(); ++itMeshData ) {
...
bool CAdjustMeshInterference::CreateVolumeMeshes( CTextile &Textile, vector<CMesh> &YarnMeshes ) {
	...
	// Create volume mesh for each yarn as MS thesis sections 2.9.1 & 2.9.2
	for (int i=0; i<iNumYarns; ++i) {
		CYarn* pYarn = Textile.GetYarn(i);
		if ( !pYarn->AddVolumeToMesh(YarnMeshes[i], *pDomain)) {
