...
void CMesh::PopulateTiXmlElement(TiXmlElement &Element, OUTPUT_TYPE OutputType) const
{
	...
	for (int i = 0; i < NUM_ELEMENT_TYPES; i++) {
		...
		for (list<int>::const_iterator itIndex = m_Indices[i].begin(); itIndex != m_Indices[i].end(); ++itIndex) {
...
int CMesh::InsertNodes(const CMesh &Mesh, XYZ Offset) {
	...
	for (XYZ node : Mesh.m_Nodes) {
		m_Nodes.push_back(node + Offset);
	}
	return iOffset;
}
...
void CMesh::InsertMesh(const CMesh &Mesh, XYZ Offset) {
	...
	for (int i = 0; i < NUM_ELEMENT_TYPES; i++) {
		for (int index : Mesh.m_Indices[i]) {
			m_Indices[i].push_back(index + iOffset);
		}
	}
}
...
void CMesh::ChangeNodeIndices(int iChangeTo, int iChangeFrom) {
	for (int i = 0; i < NUM_ELEMENT_TYPES; i++) {
		for (list<int>::iterator itIndex = m_Indices[i].begin(); itIndex != m_Indices[i].end(); ++itIndex) {
...
void CMesh::RemoveOpposingTriangles()
{
	...
	int i1[3];
	int i2[3];
	list<int>::iterator itIter1;
	list<int>::iterator itIter2;
	list<int>::iterator itTriStart1;
	list<int>::iterator itTriStart2;
...
void CMesh::RemoveOpposingQuads()
{
	...
	list<int>::iterator itIter1;
	list<int>::iterator itIter2;
	list<int>::iterator itQuadStart1;
	list<int>::iterator itQuadStart2;
...
void CMesh::RemoveDegenerateTriangles() {
	list<int> &TriangleIndices = m_Indices[TRI];
	for (list<int>::iterator itIter = TriangleIndices.begin(); itIter != TriangleIndices.end(); ) {
		list<int>::iterator itTriStart = itIter;
		int i1 = *(itIter++);
		int i2 = *(itIter++);
		int i3 = *(itIter++);
...
void CMesh::RemoveDuplicateElements(CMesh::ELEMENT_TYPE ElementType)
{
	...
	vector<int> Index1;
	vector<int> Index2;
	list<int>::iterator itStart1;
	list<int>::iterator itStart2;

	for (list<int>::iterator itIter1 = ElementIndices.begin(); itIter1 != ElementIndices.end(); )
	{
		itStart1 = itIter1;
		Index1.clear();
		...
		for (list<int>::iterator itIter2 = itIter1; itIter2 != ElementIndices.end(); )
		{
			itStart2 = itIter2;
			Index2.clear();
...
pair<XYZ, XYZ> CMesh::GetAABB(double dGrowDistance) const {
	pair<XYZ, XYZ> AABB;
	for (XYZ node : m_Nodes) {
		if (AABB.first != NULL && node != NULL) {
			AABB.first = Min(AABB.first, node);
		}
		else if (node != NULL) {
			AABB.first = XYZ(node.x, node.y, node.z);
		}
		if (AABB.second != NULL && node != NULL) {
			AABB.second = Max(AABB.second, node);
		}
		else if (node != NULL) {
			AABB.second = XYZ(node.x, node.y, node.z);
		}
	}
	if (AABB.first != NULL) {
		...
		AABB.first = XYZ(-dGrowDistance, -dGrowDistance, -dGrowDistance);
	}
	if (AABB.second != NULL) {
		...
		AABB.second = XYZ(dGrowDistance, dGrowDistance, dGrowDistance);
	}
	return AABB;
}
...
int CMesh::MergeNodes(const double TOL)
{
	...
	int iNode = 0;
	for (XYZ node : m_Nodes) {
		NodesWithIndices[iNode] = pair<int, XYZ>(iNode, node);
...
void CMesh::GetNodePairs(XYZ Vector, vector<pair<int, int> > &NodePairs, const double TOL ) const {
	...
	for (vector<XYZ>::const_iterator itNode2 = m_Nodes.begin(); itNode2 != m_Nodes.end(); ++itNode2 ) {
	...
	double dTolSquared = TOL*TOL;
	bool bFound;
	for (vector<XYZ>::const_iterator itNode1 = m_Nodes.begin(); itNode1 != m_Nodes.end(); ++itNode1, ++iNode1) {
		if (bProgress && !(iNode1 % iProgressInc)) {
			TGLOG( "GetNodePairs " << iNode1/iProgressInc*10 << "% complete");
		}
		bFound = false;
		// Matching node normally later in mesh than itNode1 so start checking here to improve speed
		for (size_t iNode2 = iNode1; iNode2 < OffsetNodes.size(); iNode2++) {
		...
		if (!bFound) {
			// Check from beginning if node earlier in mesh than one checking against
			for (size_t iNode2 = 0; iNode2 < iNode1; iNode2++) {
				if (GetLengthSquared(*itNode1, OffsetNodes[iNode2]) < dTolSquared) {
					NodePairs.push_back(make_pair(iNode1, iNode2));
					break;
				}
...
int CMesh::RemoveUnreferencedNodes() {
	...
	for (int i = 0; i < NUM_ELEMENT_TYPES; ++i) {
		for (int index : m_Indices[i]) {
			UnreferencedNodes.erase(index);   // Get rid of the nodes which are referenced..
		}
	}
...
void CMesh::ConvertHextoQuad() {
	for (list<int>::iterator itIter = m_Indices[HEX].begin(); itIter != m_Indices[HEX].end(); ) {
		int i1 = *(itIter++);
		int i2 = *(itIter++);
		int i3 = *(itIter++);
		int i4 = *(itIter++);
...
void CMesh::ConvertWedgeto2D() {
	for (list<int>::iterator itIter = m_Indices[WEDGE].begin(); itIter != m_Indices[WEDGE].end(); ) {
		int i1 = *(itIter++);
		int i2 = *(itIter++);
		int i3 = *(itIter++);
		int i4 = *(itIter++);
...
void CMesh::ConvertTettoTriangle() {
	for (list<int>::iterator itIter = m_Indices[TET].begin(); itIter != m_Indices[TET].end(); ) {
		int i1 = *(itIter++);
		int i2 = *(itIter++);
		int i3 = *(itIter++);
		int i4 = *(itIter++);
...
void CMesh::ConvertPyramidtoTet(bool bQuality) {
	for (list<int>::iterator itIter = m_Indices[PYRAMID].begin(); itIter != m_Indices[PYRAMID].end(); ) {
		int i0 = *(itIter++);
		int i1 = *(itIter++);
		int i2 = *(itIter++);
		int i3 = *(itIter++);
...
void CMesh::Translate(XYZ Vector) {
	for (vector<XYZ>::iterator itNode = m_Nodes.begin(); itNode != m_Nodes.end(); ++itNode) {
		*itNode += Vector;
	}
}
...
int CMesh::OutputElements(ostream &Output, CMesh::ELEMENT_TYPE ElementType, int iStartIndex, int iIndexOffset, string Seperator, bool bSCIRun) const
{
	...
	for (itIndex = m_Indices[ElementType].begin(); itIndex != m_Indices[ElementType].end(); ++iElementNumber)
	{
		...
		for (int i=0; i<GetNumNodes(ElementType); ++i, ++itIndex) {
			if (i > 0) {
				Output << Seperator;
			}
...
void CMesh::MeshConvexHull()
{
	...
	do
	{
		...
		for (i=0; i<(int)m_Nodes.size(); ++i)
		{
			...
			for (list<pair<int, int> >::iterator itEdge = EdgeStack.begin(); itEdge != EdgeStack.end(); ++itEdge)
			{
				int i1 = itEdge->first;
				int i2 = itEdge->second;
				int i3 = i;
...
void CMesh::BuildGrid(XYZ Min, XYZ Max, double dPointsPerUnit) {
	int iNumX = Round((Max.x-Min.x)*dPointsPerUnit);
	int iNumY = Round((Max.y-Min.y)*dPointsPerUnit);
	int iNumZ = Round((Max.z-Min.z)*dPointsPerUnit);
	BuildGrid(Min, Max, iNumX, iNumY, iNumZ);
}

const vector<XYZ> CMesh::getVertices() const {
	for (int i = 0; i < NUM_ELEMENT_TYPES; ++i) {
		if (i != TRI && !m_Indices[i].empty()) {
			CMesh TriMesh(*this);
			TriMesh.ConvertToTriangleMesh();
			return TriMesh.getVertices();
		}
	}
	vector<XYZ> vertices;
	for (int index : m_Indices[TRI]) {
		vertices.push_back(m_Nodes[index]);
	}
	return vertices;
}

const list<TriangleFace> CMesh::getFaces() const {
	list<TriangleFace> faces;
	vector<XYZ> vertices = getVertices();
	for (vector<XYZ>::const_iterator itIndex = vertices.begin(); itIndex != vertices.end(); ) {
		TriangleFace face;
		face.pointA = *(itIndex++);
		face.pointB = *(itIndex++);
		face.pointC = *(itIndex++);
...
bool CMesh::SaveToSTLAscii(string Filename) const {
	list<TriangleFace> faces = getFaces();
	ofstream Output;
	...
	for (TriangleFace face : faces) {
		...
		XYZ Normal = CrossProduct(T2-T1, T3-T1);
		Normalise(Normal);
...
bool CMesh::SaveToSTLBinary(string Filename) const {
	list<TriangleFace> faces = getFaces();
	ofstream Output;
	AddExtensionIfMissing(Filename, ".stl");
	...
	for (TriangleFace face : faces) {
		...
		Output.write((char*) &normalX, sizeof(float));
		float normalY = (float)Normal.y;
		Output.write((char*) &normalY, sizeof(float));
		float normalZ = (float)Normal.z;
		Output.write((char*) &normalZ, sizeof(float));

		float T1X = (float)T1.x;
		Output.write((char*) &T1X, 4);
		float T1Y = (float)T1.y;
		Output.write((char*) &T1Y, 4);
		float T1Z = (float)T1.z;
		Output.write((char*) &T1Z, 4);

		float T2X = (float)T2.x;
		Output.write((char*) &T2X, 4);
		float T2Y = (float)T2.y;
		Output.write((char*) &T2Y, 4);
		float T2Z = (float)T2.z;
		Output.write((char*) &T2Z, 4);

		float T3X = (float)T3.x;
		Output.write((char*) &T3X, 4);
		float T3Y = (float)T3.y;
		Output.write((char*) &T3Y, 4);
		float T3Z = (float)T3.z;
		Output.write((char*) &T3Z, 4);
