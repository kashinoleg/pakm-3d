...
: CTextileWeave2D(iWidth, iHeight, dSpacing, dThickness, bRefine, bInPlaneTangents ) {
	setShearAngle(ShearAngle);
	setShearedDomain(false);
}
...
: CTextileWeave2D(Element) {
	double shear;
	Element.Attribute("ShearAngle", &shear);
	setShearAngle(shear);
}
...
void CShearedTextileWeave2D::PopulateTiXmlElement(TiXmlElement &Element, OUTPUT_TYPE OutputType) {
	...
	Element.SetAttribute("ShearAngle", stringify(getShearAngle()));
}
...
CDomainPlanes CShearedTextileWeave2D::GetDefaultDomain( bool bSheared, bool bAddedDomainHeight ) {
	...
	if (bSheared) {
		...
		// Work out minimum x taking into account shear angle
	...
	double cosAng = cos(getShearAngle());
	double sinAng = sin(getShearAngle());
...
void CShearedTextileWeave2D::GetYSpacings() const {
	m_YSpacing.clear();
	double sinAngle = sin(getShearAngle());
	double cosAngle = cos(getShearAngle());
	for (int i = 0; i < m_iNumXYarns; i++) {
		XY spacing;
	...
	for (int iDir=0; iDir<2; ++iDir)
	{
		...
		for ( i = 0; i < iTransverseNum; ++i )
		{
			...
			for (j=0; j<iLongitudinalNum; ++j)
			{
				...
				double dDistToLongYarnEdge = 0.5 * dLongWidth / cos(getShearAngle());
				...
				double dDistToNextLongYarnEdge = 0.5 * dNextLongWidth / cos(getShearAngle());
...
double CShearedTextileWeave2D::CalcSinAngle( double R, double Width, double Height ) const
{
	...
	if ( R <= 0.25 )
		denom = (1 - 4*R - 0.2*sin(getShearAngle()));
	else
		denom = (1 - 4*R + 0.2*sin(getShearAngle()));
	denom *= denom;
	denom = 1/denom - 1;
	...
	return sin(getShearAngle())/denom;
}
		...
		for (i=0; i<iTransverseNum; ++i)
		{
			...
			for (j=0; j<iLongitudinalNum; ++j)
			{
				for (itpYarn = (*pLongitudinalYarns)[j].begin(); itpYarn != (*pLongitudinalYarns)[j].end(); ++itpYarn)
				{
					...
					double DistToEdgeU = (0.5 * (*pTransverseData)[i].dWidth / cos(getShearAngle()) )/ dDistBetweenNodes;
