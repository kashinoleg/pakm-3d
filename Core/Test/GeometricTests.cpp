...
namespace UnitTest
{
	...
	public:
		...
		TEST_METHOD(TestPointInsideYarn)
		{
		...
		TEST_METHOD(TestLenticularSection)
		{
			double dWidth = 2;
			double dHeight = 1;
		...
		TEST_METHOD(TestQuaternionRotation)
		{
			XYZ Vec(1, 0, 0);
			WXYZ Rot(PI / 2, 0, 0);
			XYZ Expected(0, 1, 0);
		...
		TEST_METHOD(TestConvertRotation)
		{
		...
		TEST_METHOD(TestGetClosestPointFunctions)
		{
			vector<XY> Points;
			int index = -1;
			XY Expected(1, -1);
		...
		TEST_METHOD(Test3DGetClosestPointFunctions)
		{
			vector<XYZ> Points;
			int index = -1;
			XYZ Expected(1, -1, -1);
		...
		TEST_METHOD(TestFindClosestSurfacePoint)
		{
			// Build a single yarn of diameter 1, going from (0,0,0) to (1,0,0)
			// The domain is 1 x 1 x 1 with center (1,1,1)
			// Build a grid of points inside the domain and check that the analytical solution
