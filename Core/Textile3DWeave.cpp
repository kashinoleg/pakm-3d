...
//			PatternElement.SetAttribute("index", j);
	...
	for (size_t i = 0; i < m_XYarnData.size(); i++) {
	...
	for (size_t i = 0; i < m_YYarnData.size(); i++) {
	...
	if (OutputType == OUTPUT_STANDARD || OutputType == OUTPUT_FULL) {
		for (size_t i = 0; i < m_XYarns.size(); i++) {
			TiXmlElement XYarns("XYarns");
			XYarns.SetAttribute("index", i);
			for (int jYarn : m_XYarns[i]) {
				TiXmlElement XYarn("XYarn");
				XYarn.SetAttribute("yarnindex", jYarn);
				XYarns.InsertEndChild(XYarn);
			}
			Element.InsertEndChild(XYarns);
		}
		for (size_t i = 0; i < m_YYarns.size(); i++) {
			TiXmlElement YYarns("YYarns");
			YYarns.SetAttribute("index", i);
			for (int jYarn : m_YYarns[i]) {
				TiXmlElement YYarn("YYarn");
				YYarn.SetAttribute("yarnindex", jYarn);
...
bool CTextile3DWeave::BuildTextile() const {
	...
	for (int j=0; j<m_iNumYYarns; ++j) {
		...
		for (int i = 0; i <= NumXYarns; i++) {
			...
			for (int k=0; k<(int)Cell.size(); ++k) {
				...
				else if ( Cell[k] == PATTERN3D_XYARN && k > 0 ) {
					// Don't adjust z if it's the binder yarn
	...
	for (int i=0; i<m_iNumXYarns; ++i) {
		...
		for (int index : m_XYarns[i]) {
			m_Yarns[index].AssignSection(CYarnSectionConstant(Section));
			int iType = IsBinderYarn(i) ? BINDER : WARP;
			SetYarnProperties( m_Yarns[index], iType );
		}
	}
	for (int i=0; i<m_iNumYYarns; ++i) {
		...
		for (int index : m_YYarns[i]) {
			m_Yarns[index].AssignSection(CYarnSectionConstant(Section));
			SetYarnProperties( m_Yarns[index], WEFT );
		}
	}
...
double CTextile3DWeave::GetWidth() const {
	double dWidth = 0;
	for (YARNDATA yarnData : m_YYarnData){
		dWidth += yarnData.dSpacing;
	}
	return dWidth;
}
...
double CTextile3DWeave::GetHeight() const {
	double dHeight = 0;
	for (YARNDATA yarnData : m_XYarnData) {
		dHeight += yarnData.dSpacing;
	}
	return dHeight;
}
...
vector<CYarn*> CTextile3DWeave::GetXYarns(int iIndex) {
	...
	for (int index : m_XYarns[iIndex]) {
		Yarns.push_back(&m_Yarns[index]);
	}
	return Yarns;
}
...
bool CTextile3DWeave::NoYarnCell(vector<PATTERN3D> &Cell) {
	for (PATTERN3D cell : Cell) {
		if (cell != PATTERN3D_NOYARN) {
			return false;
		}
	}
	return true;
