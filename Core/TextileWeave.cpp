...
bool CTextileWeave::Valid() const {
	...
	for (int i = 0; i < m_iNumXYarns; i++) {
		...
		for (int j = 0; j < m_iNumYYarns; j++) {
			int iNumYarns = 0;
			for (PATTERN2D cell : GetCell(j, i)) {
				if (cell == PATTERN_XYARN) {
					++iNumYarns;
				}
			}
			if (j==0) {
...
bool CTextileWeave::BuildTextile() const {
	...
	for (int i = 0; i < m_iNumXYarns; i++) {
		...
		for (int j = 0; j <= m_iNumYYarns; j++) {
			...
			m_XYarns[i] = Yarns;
			iYarn = 0;
			z = m_dFabricThickness / (2 * Cell.size());// -m_dGapSize / 2;
			for (PATTERN2D cell : Cell) {
				if (cell == PATTERN_XYARN) {
					m_Yarns[Yarns[iYarn]].AddNode(CNode(XYZ(x, y, z), XYZ(1, 0, 0)));
					++iYarn;
				}
				z += m_dFabricThickness / Cell.size();// +m_dGapSize / 2;
			}
			if (j < m_iNumYYarns) {
	...
	dWidth = GetWidth();
	dHeight = GetHeight();
	for (vector<CYarn>::iterator itYarn = m_Yarns.begin(); itYarn != m_Yarns.end(); ++itYarn) {
...
void CTextileWeave::CorrectInterference() const
{
	...
	for (int iDir = 0; iDir<2; ++iDir)
	{
		...
		for (int i=0; i<iTransverseNum; ++i)
		{
			TransverseYarnsMesh.Clear();
			for (vector<int>::iterator itpYarn = (*pTransverseYarns)[i].begin(); itpYarn != (*pTransverseYarns)[i].end(); ++itpYarn)
			{
				m_Yarns[*itpYarn].AddSurfaceToMesh(TransverseYarnsMesh, RepeatLimits);
			}
			TransverseYarnsMesh.Convert3Dto2D();
			...
			{
				for (int yarn : (*pLongitudinalYarns)[j])
				{
					YarnPosInfo.iSection = i;
					YarnPosInfo.dSectionPosition = 0;
					YarnPosInfo.SectionLengths = m_Yarns[yarn].GetYarnSectionLengths();

					pInterpolation = m_Yarns[yarn].GetInterpolation();
					Node = pInterpolation->GetNode(m_Yarns[yarn].GetMasterNodes(), i, 0);
					XYZ Up = Node.GetUp();
					XYZ Side = CrossProduct(Node.GetTangent(), Up);
					...
					XYZ Center = m_Yarns[yarn].GetMasterNodes()[i].GetPosition();
					Modifiers.clear();
					for (vector<XY>::iterator itPoint = Points.begin(); itPoint != Points.end(); ++itPoint) {
						XYZ P = itPoint->x * Side + itPoint->y * Up + Center;
						if (TransverseYarnsMesh.IntersectLine(Center, P, Intersections, make_pair(true, false))) {
							double dU = Intersections[0].first;
							XYZ Normal = Intersections[0].second;
							double dProjectedGap = m_dGapSize / DotProduct(Normal, Center-P);
	...
	for (int i=0; i<(int)m_Yarns.size(); ++i)
	{
		...
		YarnPosInfo.dSectionPosition = 0;
		for (int j=0; j<(int)YarnSectionModifiers[i].size(); ++j)
		{
			YarnPosInfo.iSection = j;

			vector<XY> Points = pYarnSection->GetSection(YarnPosInfo, YarnSectionModifiers[i][j].size());
