...
void CYarn::PopulateTiXmlElement(TiXmlElement &Element, OUTPUT_TYPE OutputType) {
	...
	for (vector<XYZ>::iterator itXYZ = m_Repeats.begin(); itXYZ!=m_Repeats.end(); ++itXYZ) {
		TiXmlElement Repeat("Repeat");
		Repeat.SetAttribute("value", stringify(*itXYZ));
		Element.InsertEndChild(Repeat);
	}
	for (size_t i = 0; i < m_MasterNodes.size(); i++) {
	...
	if (OutputType == OUTPUT_FULL) {
		...
		for (int i=0; i<(int)m_SectionLengths.size(); ++i) {
...
void CYarn::AddNode(const CNode &Node) {
	...
	for (CNode node : m_MasterNodes) {
		if (node.GetPosition() == NodePos) {
...
bool CYarn::ReplaceNode(int iIndex, CNode NewNode) {
	...
	for ( itNode = m_MasterNodes.begin(); itNode != m_MasterNodes.end(); ++itNode, ++i ) {
		if ( itNode->GetPosition() == NodePos && i != iIndex ) {
			// If i == iIndex position might be same but tangent or Up might have changed so still want to replace
...
bool CYarn::BuildSlaveNodes() const {
	...
	for (size_t i = 0; i < m_MasterNodes.size() - 1; i++) {
...
bool CYarn::AddAABBToMesh(CMesh &Mesh, const vector<pair<int, int> > &RepeatLimits) const {
	...
	for (itRepeat = m_Repeats.begin(), itLimits = RepeatLimits.begin();
		itRepeat != m_Repeats.end() && itLimits != RepeatLimits.end(); ++itRepeat, ++itLimits) {
...
vector<vector<XYZ>> CYarn::getPoints() const {
	vector<vector<XYZ>> points;
	// If the yarn needs building then build it before creating the mesh
	if (!BuildYarnIfNeeded(SURFACE)) {
		return points;
	}
	...
	size_t iNumPoints = m_SlaveNodes[0].GetSectionPoints().size();
	for (CSlaveNode node : m_SlaveNodes) {
		if (iNumPoints != node.GetSectionPoints().size()) {
	...
	// Add nodes to the mesh
	for (CSlaveNode node : m_SlaveNodes) {
		vector<XYZ> section(node.GetSectionPoints());
...
bool CYarn::AddSurfaceToMesh(CMesh &Mesh, bool bAddEndCaps) const {
	...
	for (vector<XYZ> section : getPoints()) {
		for (XYZ point : section) {
			Mesh.AddNode(point);
		}
	}

	// Create the surface mesh elements
	for (size_t i = 0; i < m_SlaveNodes.size() - 1; i++) {
		size_t iNumPoints = m_SlaveNodes[i].GetSectionPoints().size();
		for (size_t j = 0; j < iNumPoints; j++) {
			vector<int> Indices;
			...
			if (v == iNumPoints)
				v = 0;
			Indices.push_back(iFirstNode+v+u*iNumPoints);
...
bool CYarn::AddSurfaceToMesh(CMesh &Mesh, const vector<XYZ> &Translations, bool bAddEndCaps) const {
	CMesh SingleYarnMesh;
	if (!AddSurfaceToMesh(SingleYarnMesh, bAddEndCaps)) {
		return false;
	}
	CMesh FinalYarnMesh;
	for (XYZ translation : Translations) {
		FinalYarnMesh.InsertMesh(SingleYarnMesh, translation);
	}
...
void CYarn::StraightenYarn(double dStraightness) {
	...
	XYZ Axis;
	WXYZ Rotation;
	for (int i=1; i<(int)m_MasterNodes.size()-1; ++i) {
...
bool CYarn::PointInsideYarn(const XYZ &Point, XYZ *pTangent, XY *pLoc, double* pVolumeFraction,
	double* pDistanceToSurface, double dTolerance, XYZ *pOrientation, XYZ *pUp) const {
	//PROFILE_FUNC()
	if (!BuildYarnIfNeeded(SURFACE)) {
		return false;
	}
	...
	if (pOrientation && !bSectionConstant) {
		if (!BuildYarnIfNeeded(VOLUME)) {
			// Only need to build the volume mesh if pOrientation == true and varying sections
			return false;
		}
	}
	...
	//		cout << "Num iterations: " << iIterations << endl;
			...
			if ( bIsInside ) {
				...
				if (pVolumeFraction) {
					...
					{
						double dFibreArea = GetFibreArea(); //GetFibreArea(m_pParent->GetGeometryScale()+"^2");
						/*if (dFibreArea == 0)
...
bool CYarn::PointInsideYarn(const XYZ &Point, const vector<XYZ> &Translations, XYZ *pTangent,
	XY* pLoc, double *pVolumeFraction, double* pDistanceToSurface,
	double dTolerance, XYZ *pOrientation, XYZ *pUp) const {
	// TODO OPTIMISE THIS
...
bool CYarn::FindPlaneContainingPoint(const XYZ &Point, double &u, double dTolerance, int iSeg) const {
	...
	if (d1 >= -dConvergenceTolerance && d2 >= -dConvergenceTolerance) {
		if (d1 == 0.0) {
			// Point lies on one of end planes if d1 or d2 == 0.0
			u = 0.0;
		}
		else if (d2 == 0.0) {
			u = 1.0;
...
bool CYarn::FindClosestSurfacePoint(const XYZ &Point, const vector<XYZ> &Translations, XYZ &SurfacePoint,
	int iNumSectionPoints, double dTolerance) {
	vector<XYZ>::const_iterator itXYZ;
	double minDist = -1, dist = -1.0;
	XYZ SPoint;
	for (itXYZ = Translations.begin(); itXYZ != Translations.end(); ++itXYZ) {
		dist = FindClosestSurfacePoint(Point - *itXYZ, SPoint, iNumSectionPoints, dTolerance);
		if ( fabs(dist + 1.0) > 1e-9 ) {
			// Add 1 because FindClosestSurfacePoint returns -1 if unsuccessful
			if ( dist < minDist || minDist == -1 ) {
				minDist = dist;
				SPoint += *itXYZ;
				SurfacePoint = SPoint;
...
double CYarn::FindClosestEdgeDistance( XY &Loc, const vector<XY> &SectionPoints, double dTolerance ) const
{
	...
	while (itP1 != SectionPoints.end())
	{
		...
		if ( dEdgeDistance < dTolerance ) {
			// Point on innner side so check distance. 
			if (bStart) {
