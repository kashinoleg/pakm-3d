...
{
	char codeModule[] = "code";
	char codeClass[] = "InteractiveConsole";
	m_pConsoleInstance = CreateInstance(codeModule, codeClass);
	...
	if (m_pConsoleInstance)
	{
		...
		if (pNameSpace)
		{
			...
			char szModule[] = "TexGen.Completer";
			char csClass[] = "Completer";
			m_pCompleterInstance = CreateInstance(szModule, csClass, pArgs);
...
/*bool CPythonWrapper::SendCommand(string Command)
{
	...
	if ( CTexGen::Instance().GetMessagesOn() )
		CTexGen::Instance().SetLogger(CLoggerGUI(true));
