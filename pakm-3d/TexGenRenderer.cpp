...
void CTexGenRenderer::RenderTextile(string TextileName, double dOpacity)
{
	if (TextileName == "")
	{
		...
		for (itTextile = CTexGen::Instance().GetTextiles().begin(); itTextile != CTexGen::Instance().GetTextiles().end(); ++itTextile)
		{
	...
	//CTexGen::GetInstance().PrintTextileVolume(*pTextile);
...
void CTexGenRenderer::RenderTextile(CTextile &Textile, double dOpacity)
{
	...
	BeginBatch();
	int i = 0;
	for (auto yarn = Textile.GetYarns().begin(); yarn != Textile.GetYarns().end(); yarn++)
	{
		pProp = RenderYarn(*yarn, pDomain, GetIndexedColor(i), dOpacity);
		if (pProp)
		{
			PROP_YARN_INFO Info;
			Info.iYarn = i;
...
void CTexGenRenderer::RenderMesh(CTextile &Textile)
{
	...
	for (auto yarn = Textile.GetYarns().begin(); yarn != Textile.GetYarns().end(); yarn++)
	{
		pProp = RenderYarnMesh(*yarn, pDomain, GetIndexedColor(i));
		if (pProp)
		{
			PROP_YARN_INFO Info;
			Info.iYarn = i;
...
{
	BeginBatch();
	for (size_t i = 0; i < Textile.GetYarns().size(); i++)
	{
...
void CTexGenRenderer::RenderGrid(CTextile &Textile, int iResX, int iResY, int iResZ, double dLengthModifier)
{
	...
	else  // Find the limits of the yarns
	{
		...
		{
			pair<XYZ, XYZ> YarnLimits = yarn->GetAABB();
