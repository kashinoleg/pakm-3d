...
#define oprev(otri1, otri2)                                                   \
	sym(otri1, otri2);                                                          \
	otri2.orient = plus1mod3[otri2.orient];
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, double area) {
	double dxoa = triorg[0] - triapex[0];
	double dxda = tridest[0] - triapex[0];
	double dxod = triorg[0] - tridest[0];
	double dyoa = triorg[1] - triapex[1];
	double dyda = tridest[1] - triapex[1];
	double dyod = triorg[1] - tridest[1];
...
void poolrestart(memorypool *pool)
{
	...
	pool->nextitem = (void *)(alignptr + (unsigned long)pool->alignbytes - (alignptr % (unsigned long)pool->alignbytes));
...
triangle *triangletraverse(CMesh *m)
{
	...
	do {
		...
		//Skip dead ones.
	} while (newtriangle[1] == (triangle)NULL);
	return newtriangle;
}
...
double orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
	double aheight, double bheight, double cheight, double dheight, double permanent) {
	double adx;
	double bdx;
	double cdx;
	double ady;
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			shouldbedelaunay = (oppotri.tri != m->dummytri) &&
				oppotri.tri[1] != (triangle)NULL && (triangleloop.tri < oppotri.tri) &&
...
enum locateresult locate(CMesh *m, behavior *b, vertex searchpoint, otri *searchtri)
{
	...
	if (m->recenttri.tri != (triangle *)NULL) {
		if (m->recenttri.tri[1] != (triangle)NULL) {
	...
	while (totalsamplesleft > 0) {
		...
		do {
			...
			if (sampletri.tri[1] != (triangle)NULL) {
				org(sampletri, torg);
				dist = (searchpoint[0] - torg[0]) * (searchpoint[0] - torg[0]) +
					(searchpoint[1] - torg[1]) * (searchpoint[1] - torg[1]);
				if (dist < searchdist) {
	...
	if (regions > 0) {
		...
		for (i = 0; i < regions; i++) {
			if (regiontris[i].tri != m->dummytri) {
				...
				if (regiontris[i].tri[1] != (triangle)NULL) {
...
void splittriangle(CMesh *m, behavior *b, badtriang *badtri)
{
	...
	if (badotri.tri[1] != (triangle)NULL && (borg == badtri->triangorg) &&
